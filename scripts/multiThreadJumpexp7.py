...
def MainProssece(*args):
	...
	while not rospy.is_shutdown():
		global Orientationt,t,isJump,isFix
		if iniTime ==0:
			iniTime = time.time()
		t = time.time() -iniTime
		
		if t > 10 and t < 13:
			delY = -0.16 + (0.03/3.0) *(t-10)
			#delX = 0 + (-0.02)/3.0 *(t-10)
			delX= 0;
			y1,y2,y3,y4 = delY, delY, delY, delY
			x1,x2 = delX, delX
		if t > 13 and isFix == False:
			isFix = True	
			PawlControl()
		if t > 14:
			Ang9.data = -5000
		if t > 100 and t < 100.1:
			#y1,y2 = -0.1376, -0.1376
			#x1,x2 = 0.0816, 0.0816
			y1,y2 = -0.16, -0.16
			x1,x2 = 0.0, 0.0
			print t
			if t > 100.05 and isJump == False:
				isJump = True
				JumpMethod()
		...
		if t <=5:
			...
			tt =t-5			
			leg1th1 = 90 + (leg1th1_des-90)*tt/5
			leg2th1 = -90 + (leg2th1_des+90)*tt/5			
			leg3th1 = 90 + (leg3th1_des-90)*tt/5
			leg4th1 = -90 + (leg4th1_des+90)*tt/5
			leg1th2 = leg1th2_des*tt*0/5
			leg2th2 = leg2th2_des*tt*0/5
			leg3th2 = leg3th2_des*tt*0/5
			leg4th2 = leg4th2_des*tt*0/5
		elif t>10:
